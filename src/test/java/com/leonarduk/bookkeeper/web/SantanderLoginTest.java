...
public class SantanderLoginTest {
    ...
    public final void testIsLoaded() {
        ...
        Mockito.when(questionNode.getAttribute(Matchers.anyString()))
                .thenReturn("1");
        Mockito.when(
                this.webDriver.findElement(By
                        .xpath(SantanderLogin.QUESTION_XPATH))).thenReturn(
                questionNode);
        final WebElement numberNode = idSubmit;
        Mockito.when(
                this.webDriver.findElement(By
                        .xpath(SantanderLogin.CUSTOMER_ID_XPATH))).thenReturn(
                numberNode);

        Mockito.when(
                this.webDriver.findElement(By
                        .xpath(SantanderLogin.CUSTOMER_ID_SUBMIT_XPATH)))
                .thenReturn(idSubmit);

        final WebElement answerField = Mockito.mock(WebElement.class);
        Mockito.when(
                this.webDriver.findElement(By
                        .xpath(SantanderLogin.ANSWER_XPATH))).thenReturn(
                answerField);

        final WebElement answerSubmitField = Mockito.mock(WebElement.class);
        Mockito.when(
                this.webDriver.findElement(By
                        .xpath(SantanderLogin.ANSWER_SUBMIT_XPATH)))
                .thenReturn(answerSubmitField);
        final int numberOfDigits = 3;
        for (int i = 1; i <= numberOfDigits; i++) {
            final WebElement signPosition = Mockito.mock(WebElement.class);
            Mockito.when(signPosition.getAttribute("tabindex")).thenReturn(
                    String.valueOf(i));
            Mockito.when(
                    this.webDriver.findElement(By
                            .name(SantanderLogin.SIGN_POSITION_PREFIX + i)))
                    .thenReturn(signPosition);
            final String xpath =
                    SantanderLogin.PASSWORD_INDEX_XPATH + (i + 1) + "]";
        ...
        Mockito.when(
                this.webDriver.findElement(By
                        .xpath(SantanderLogin.PASSWORD_SUBMIT_XPATH)))
